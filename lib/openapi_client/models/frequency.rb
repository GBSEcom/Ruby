  ...
  class Frequency
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Frequency` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Frequency`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @every > 1000
      return false if @every < 1
      return false if @unit.nil?
      unit_validator = EnumAttributeValidator.new('String', ["DAY", "WEEK", "MONTH", "YEAR"])
      return false unless unit_validator.valid?(@unit)
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] every Value to be assigned
    def every=(every)
      if every.nil?
        fail ArgumentError, 'every cannot be nil'
      end

      if every > 1000
        fail ArgumentError, 'invalid value for "every", must be smaller than or equal to 1000.'
      end

      if every < 1
        fail ArgumentError, 'invalid value for "every", must be greater than or equal to 1.'
      end
