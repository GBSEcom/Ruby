  ...
  class UnionPayAuthenticationRequestAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::UnionPayAuthenticationRequestAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::UnionPayAuthenticationRequestAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] sms_phone_number Value to be assigned
    def sms_phone_number=(sms_phone_number)
      if sms_phone_number.nil?
        fail ArgumentError, 'sms_phone_number cannot be nil'
      end

      if sms_phone_number.to_s.length < 7
        fail ArgumentError, 'invalid value for "sms_phone_number", the character length must be great than or equal to 7.'
      end

      pattern = Regexp.new(/^(?!\s*$).+/)
      if sms_phone_number !~ pattern
        fail ArgumentError, "invalid value for \"sms_phone_number\", must conform to the pattern #{pattern}."
      end
