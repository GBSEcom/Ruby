...
module OpenapiClient
  ...
  class SoftDescriptor
    ...
    def dynamic_merchant_name=(dynamic_merchant_name)
      if dynamic_merchant_name.nil?
        fail ArgumentError, 'dynamic_merchant_name cannot be nil'
      end

      pattern = Regexp.new(/^(?!\s*$).+/)
      if dynamic_merchant_name !~ pattern
        fail ArgumentError, "invalid value for \"dynamic_merchant_name\", must conform to the pattern #{pattern}."
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] customer_service_number Value to be assigned
    def customer_service_number=(customer_service_number)
      if !customer_service_number.nil? && customer_service_number.to_s.length > 10
        fail ArgumentError, 'invalid value for "customer_service_number", the character length must be smaller than or equal to 10.'
      end

      pattern = Regexp.new(/^[0-9]+$/)
      if !customer_service_number.nil? && customer_service_number !~ pattern
        fail ArgumentError, "invalid value for \"customer_service_number\", must conform to the pattern #{pattern}."
      end
