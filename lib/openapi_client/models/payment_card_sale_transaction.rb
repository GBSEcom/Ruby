...
module OpenapiClient
  ...
  class PaymentCardSaleTransaction < PrimaryTransaction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PaymentCardSaleTransaction` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PaymentCardSaleTransaction`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [payment_method, stored_credentials, settlement_split, create_token, currency_conversion, authentication_request, authentication_result].hash
    end
    ...
    def to_hash(attribute_map = self.class.attribute_map, openapi_nullable = Set.new([]))
      ...
      # todo: error: self.class.superclass.openapi_nullable could not be resolved
