  ...
  class EncryptedApplePay
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::EncryptedApplePay` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::EncryptedApplePay`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'application_data')
        self.application_data = attributes[:'application_data']
      end
    ...
    def valid?
      ...
      return false if @signature !~ Regexp.new(/^(?!\s*$).+/)
      version_validator = EnumAttributeValidator.new('String', ["EC_v1"])
      return false unless version_validator.valid?(@version)
      return false if @merchant_id.nil?
      return false if @merchant_id !~ Regexp.new(/^(?!\s*$).+/)
      true
    end
    ...
    def version=(version)
      validator = EnumAttributeValidator.new('String', ["EC_v1"])
    ...
    def hash
      [data, header, signature, version, application_data, merchant_id].hash
    end
