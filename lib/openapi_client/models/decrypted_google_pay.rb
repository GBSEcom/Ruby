  ...
  class DecryptedGooglePay
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::DecryptedGooglePay` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::DecryptedGooglePay`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@cryptogram.nil? && @cryptogram !~ Regexp.new(/^(?!\s*$).+/)
      return false if !@eci_indicator.nil? && @eci_indicator !~ Regexp.new(/[0-9]{2}/)
      true
    end
    ...
    def hash
      [account_number, expiration, cardholder_name, brand, cryptogram, eci_indicator, stored_credentials].hash
    end
